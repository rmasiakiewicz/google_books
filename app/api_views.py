...
def books_list():
    ...
    for key in request.args.keys():
        if key not in allowed_parameters:
            return (
                jsonify(
                    error=True, message="Unexpected query string: %s" % key, items=[]
                ),
    ...
    if builder is None:
        return (
            jsonify(
                error=True, message="Bad date format, expected YYYY-MM-DD", items=[]
            ),
            HTTPStatus.BAD_REQUEST,
        )
    data = builder.paginate(page=page, per_page=per_page)
    return (
        jsonify(error=False, message="ok", items=data.items, total_items=data.total),
        HTTPStatus.OK,
    )
